...
<div class="container">
	<div class="row">
		<!-- 댓글 데이터 전송 -->
		<form method="post" name="TPC_writeform"
			action="/TDL/TDL_COMMENT_writePro.do" onsubmit="return writeSave()">
			<!-- TPC_num에는 자유게시물 번호가 저장된다. -> TP_num -->

			<input type="hidden" name="TP_num" value="${TP_num }">
			<input type="hidden" name="TP_num" value="${pageNum }">
			<input type="hidden" name="TP_num" value="${article}">
			<input type="hidden" name="TP_num" value="${TPC_num}">
			<input type="hidden" name="TP_num" value="${pgListC}">
			<input type="hidden" name="TP_num" value="${articleListC}">
			<table class="table table-striped"
				style="text-align: center; border: 1px solid #dddddd">
				<thead>
		...
		<div class="container">
				...
				-->
					...
					<c:forEach var="articleC" items="${articleListC}">
						<tr>
							...
							<!-- 게시물번호 -->
							<a
								href="/TDL/TPC_COMMENT_content.do?TPC_num=${articleC.TPC_num}&pageNumC=${pgListC.currentPage}"></a>
							...
							<td align="left">[${articleC.TPC_id }]<br>
								...
								${articleC.TPC_date.substring(4,6)}월
								${articleC.TPC_date.substring(6,8) }일 작성
							</td>

							<td align="right"><button class="btn btn-primary">${articleC.TPC_good }</button>
								<button class="btn btn-danger">${articleC.TPC_bad }</button>
						</tr>
					</c:forEach>
				</table>
			</c:if>
			...
			<c:if test="${pgListC.startPage > pgListC.blockSize}">
				<a
					href="/TDL/TDL_POST_content.do?TP_num=${article.TP_num}&pageNum=${pgList.currentPage}&pageNumC=${pgListC.startPage-pgListC.blockSize}">[이전]</a>
			</c:if>

			<c:forEach var="i" begin="${pgListC.startPage}"
				end="${pgListC.endPage}">
				<a
					href="/TDL/TDL_POST_content.do?TP_num=${article.TP_num}&pageNum=${pgList.currentPage}&pageNumC=${i}">
					<c:if test="${pgListC.currentPage==i}">
						<font color="red"><b>[${i}]</b></font>
					</c:if> <c:if test="${pgListC.currentPage!=i}">
					[${i}]
				</c:if>
				</a>
			</c:forEach>
			<c:if test="${pgListC.endPage < pgListC.pageCount }">
				<a
					href="/TDL/TDL_POST_content.do?TP_num=${article.TP_num}&pageNum=${pgList.currentPage}&pageNumC=${pgListC.startPage+pgListC.blockSize}">[다음]</a>
			</c:if>
		</div>
